  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 flex flex-col gap-xs relative z-0">
        <div className={`absolute top-0 right-3 h-[42px] flex items-center z-10 transition-opacity duration-300 ${isPinned ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
          <button
            type="button"
            onClick={togglePin}
            className="p-1 rounded hover:bg-bg-page transition-colors text-text-secondary hover:text-primary focus:outline-none"
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <span className={`ml-3 text-text-l font-medium transition-all duration-300 whitespace-nowrap overflow-hidden ${isPinned ? 'opacity-100 w-auto' : 'opacity-0 w-0 group-hover:opacity-100 group-hover:w-auto'}`}>
              {item.label}
            </span>
          </a>
        ))}
